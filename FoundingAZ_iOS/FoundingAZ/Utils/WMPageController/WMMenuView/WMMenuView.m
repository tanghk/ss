...
#pragma mark 竖线 xz添加
- (void)creatWithvLineImageV:(CGRect)frame withSpace:(CGFloat)space{
    
//    if (![TTUserDataManager getInstance].isHiddleVLine) {
        [self.scrollView addSubview:self.vLineImageV];
        self.vLineImageV.frame = CGRectMake(frame.origin.x+frame.size.width+space, (self.frame.size.height-16)/2, 1, 16);
//    }
}
...
- (UIImageView *)vLineImageV{
    ...
    if (!_vLineImageV) {
        _vLineImageV = [[UIImageView alloc] init];
        _vLineImageV.hidden = YES;
        _vLineImageV.backgroundColor = HEXACOLOR(0xC0CBDA, 1.0);
    }
...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        //xz添加
        if (i == 0) {
            [self creatWithvLineImageV:frame withSpace:contentWidth/2];
        }
    ...
    if (contentWidth < self.scrollView.frame.size.width) {
        ...
        for (int i = 0; i < self.frames.count; i++) {
            ...
            //xz添加
            if (i == 1) {
                CGRect frameOne = [self.frames[0] CGRectValue];
                CGRect frameTwo = [self.frames[1] CGRectValue];
                [self creatWithvLineImageV:frameOne withSpace:(frameTwo.origin.x-frameOne.origin.x-frameOne.size.width)/2];
            }
        }
