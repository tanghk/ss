...
#pragma mark - 私有方法
#pragma mark - 创建工具条:保存图片、删除图片、页码
- (void)createToolbar
{
    ...
    _toolbar.showDeletePicBtn = _showDeletePicBtn;
    _toolbar.showSavePicBtn = _showSavePicBtn;
    _toolbar.photos = _photos;//photos赋值要放在showDeletePicBtn、showSavePicBtn之后。
...
#pragma mark - MJPhotoToolbarDelegate: 删除图片
-(void)deletePicWithIndex:(NSInteger)index
{
    NSInteger oldIndex = _currentPhotoIndex;
    NSInteger oldCount = [_photos count];
    
    NSMutableArray *photoArr = [NSMutableArray arrayWithArray:_photos];
    [photoArr removeObjectAtIndex:index];
    _photos = [NSArray arrayWithArray:photoArr];
    if ([_photos count]>1) {
        _currentPhotoIndex = (index >= oldCount-1) ? ([_photos count]-1) : oldIndex;
    }
...
//    _currentPhotoIndex =
    ...
    [_delegate delPicWithIndex:index];
    
    for (MJPhotoView *photoView in _visiblePhotoViews) {
        if (index == kPhotoViewIndex(photoView)) {
